...
public class ContainerVm {
 ...
 /**
    ...
    public ContainerVm(
        ...
        //eunsook
        setAvailableBw(bw);
        setAvailableCpu(getPeList().size());
        setAvailableMem(ram);
...
//            Log.formatLine("Total mips usage is %.2f", requestedMipsTemp);
    ...
    public double getTotalUtilizationOfCpu(double time) {
        ...
        for (Container container : getContainerList()) {
        	Log.printLine(container.getTotalUtilizationOfCpu(time));
...
//        Log.printLine("Host: Create VM???......" + container.getId());
        ...
        setAvailableBw(getAvailableBw()-container.getBw());
        setAvailableCpu(getAvailableCpu()-container.getNumberOfPes());
        setAvailableMem(getAvailableMem()-container.getRam());
        setSize(getSize() - container.getSize());
        getContainerList().add(container);
