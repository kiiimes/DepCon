...
public class ContainerDatacenter extends SimEntity {
    ...
    public long averagePlacementTime=0;
    
    public long totalPlacementTime=0;
    ...
    public ContainerDatacenter(
            ...
            double schedulingInterval, String experimentName, String logAddress, List<Framework> frameworkList) throws Exception {
        super(name);
    ...
    public void processContainerSubmit(SimEvent ev, boolean ack) {
        ...
        for(ContainerVm containerVm : getContainerVmList()) {
        	totalCpu += containerVm.getNumberOfPes();
        	totalRam += containerVm.getRam();
        }
        Log.printLine("totalCpu : " + totalCpu);
        Log.printLine("totalRam : " + totalRam);
        
        for (Container container : containerList) {
            //boolean result = getContainerAllocationPolicy().allocateVmForContainer(getContainerVmList(),getFrameworkList());
        	//container = calculateDrf(getFrameworkList(),totalCpu,totalRam);
...
//                    Log.printConcatLine("Assigning the container#" + container.getUid() + "to VM #" + containerVm.getUid());
    ...
    protected void processVmCreate(SimEvent ev, boolean ack) {
        ...
        for(ContainerHost containerHost : getHostList()) {
        	totalCpu += containerHost.getNumberOfPes();
        	totalRam += containerHost.getRam();
        }
        

        long t1 = System.nanoTime();
        
        //containerVm = calculateDrf(getFrameworkList(),totalCpu,totalRam);
        boolean result = getVmAllocationPolicy().allocateHostForVm(containerVm);
        
        long t3 = System.nanoTime();

        count++;
        totalPlacementTime += (t3-t1);
        averagePlacementTime = totalPlacementTime/count;
        
        //Log.printLine("time!! : " +((t3 - t1)/1000000.0));
        Log.printLine("time!!! : " + (t3 - t1) + " ns");
        Log.printLine("totalTime : " + totalPlacementTime);
        Log.printLine("averageTime : " + averagePlacementTime);
    ...
    protected double predictFileTransferTime(List<String> requiredFiles) {
        double time = 0.0;
        Log.printLine("requiredFiles : " + requiredFiles);
    ...
    public ContainerVm calculateDrf(List<Framework> frameworkList, double totalCpu, double totalRam) {
  	   ...
  	   containerVm = frameworkList.get(dominantFramework).getVmList().get(0);
  	   frameworkList.get(dominantFramework).getVmList().remove(0);
    ...
    /*
    public Container calculateDrf(List<Framework> frameworkList,double totalCpu, double totalRam) {
 	  ...
 	  Log.printLine("beforesetMem? : " + frameworkList.get(dominantFramework).getRequestedMem());
 	   cpuShare = frameworkList.get(dominantFramework).getCpuShare() + (frameworkList.get(dominantFramework).getRequestedCpu()/totalCpu);
 	   memShare = frameworkList.get(dominantFramework).getMemShare() + (frameworkList.get(dominantFramework).getRequestedMem()/totalRam);
 	   
 	   Log.printLine("frameworkId : " + dominantFramework);
 	   Log.printLine("cpuSHare : " + cpuShare);
 	   Log.printLine("memSHare : " + memShare);
 	   
 	   frameworkList.get(dominantFramework).setCpuShare(cpuShare);
 	   frameworkList.get(dominantFramework).setMemShare(memShare);
 	   Log.printLine("setMem? : " + frameworkList.get(dominantFramework).getMemShare());
